...
import {
    lecturerUpdateCourseClassExercise
} from "@/utils/service/crud/LecturerService";
...
interface EditCourseClassExerciseForm {
    title: string;
    description: string;
...
export default function EditExerciseModal({ exercise, onClose }: EditExerciseModalProps) {
    ...
    const [formData, setFormData] = useState<EditCourseClassExerciseForm>({
        ...
        week_number: exercise.pivot?.week_number ?? null,
        deadline: null,
        is_active: true,
        is_test: false,
    ...
    const handleSubmit = async () => {
        ...
        const res:any = await lecturerUpdateCourseClassExercise(payload, exercise.id);
